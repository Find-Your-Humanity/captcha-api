...
def verify_abstract_captcha(req: AbstractVerifyRequest) -> Dict[str, Any]:
    ...
    if USE_REDIS and get_redis():
        ...
        # 무결성: 서명이 왔다면 모두 검사 (길이는 image_urls 길이 기준)
        if req.signatures is not None:
            image_urls = list(doc.get("image_urls", []) or [])
            if len(req.signatures) != len(image_urls):
                return {"success": False, "message": "Invalid signatures length"}
            for i, sig in enumerate(req.signatures):
                if not _verify_image_token(req.challenge_id, i, sig):
                    return {"success": False, "message": "Invalid signature detected"}

        is_positive = list(doc.get("is_positive", []) or [])
        tp = sum(1 for i, flag in enumerate(is_positive) if flag and i in selections_set)
        fp = sum(1 for i, flag in enumerate(is_positive) if (not flag) and i in selections_set)
        fn = sum(1 for i, flag in enumerate(is_positive) if flag and i not in selections_set)
        ...
        if DEBUG_ABSTRACT_VERIFY:
            try:
                print(
                    f"🧮 [abstract-verify:R] tp={tp}, fp={fp}, fn={fn}, precision={precision:.4f}, recall={recall:.4f}, "
                    f"img_score={img_score:.4f}, positives={sorted(list(positives_set))}, selections={sorted(list(selections_set))}, "
                    f"is_pass={is_pass}, attempts={attempts}"
                )
            except Exception:
                pass
...
def verify_image_grid(req: ImageGridVerifyRequest) -> Dict[str, Any]:
    ...
    if USE_REDIS and get_redis():
        ...
        try:
            print(
                f"🔎 [/api/imagecaptcha-verify:R] target={target_label} success={ok} attempts={attempts} "
